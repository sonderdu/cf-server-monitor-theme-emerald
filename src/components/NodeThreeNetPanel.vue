  ...
  <div class="flex gap-4">
    ...
    <div class="flex-1 min-w-0">
      ...
      <div class="flex flex-col gap-2">
        <div v-for="row in providerRows" :key="'l-' + row.key">
          <!-- 文字行 -->
          <div class="flex items-center text-[11px] leading-none gap-1.5 mb-0.5">
            ...
            <span class="font-medium tabular-nums ml-auto" :class="row.latencyToneClass">{{ row.latency }}</span>
    ...
    <div class="flex-1 min-w-0">
      ...
      <div class="flex flex-col gap-2">
        <div v-for="row in providerRows" :key="'s-' + row.key">
          <!-- 文字行 -->
          <div class="flex items-center text-[11px] leading-none gap-1.5 mb-0.5">
            <span class="size-2 rounded-full shrink-0" :class="providerDotColor(row.key)" />
            <span class="text-muted-foreground whitespace-nowrap">{{ row.name }}</span>
            <span class="font-medium tabular-nums ml-auto" :class="row.lossToneClass">{{ row.loss }}</span>
          </div>
          <!-- 柱条行 -->
